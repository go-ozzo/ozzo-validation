...
func TestStructRules_Validate(t *testing.T) {
	tests := []struct {
		...
		// empty rules
		{"t1", Model1{A: "abc", B: "xyz"}, StructRules{}, ""},
		{"t2", Model1{A: "abc", B: "xyz"}, StructRules{}.Add("A").Add("B"), ""},
		// normal rules
		{"t3", Model1{A: "abc", B: "xyz"}, StructRules{}.Add("A", &validateAbc{}).Add("B", &validateXyz{}), ""},
		{"t4", Model1{A: "xyz", B: "abc"}, StructRules{}.Add("A", &validateAbc{}).Add("B", &validateXyz{}), "A: error abc; B: error xyz."},
		// model pointer
		{"t5", &Model1{A: "xyz", B: "abc"}, StructRules{}.Add("A", &validateAbc{}).Add("B", &validateXyz{}), "A: error abc; B: error xyz."},
		// private properties
		{"t6", &Model1{A: "xyz", c: "abc"}, StructRules{}.Add("c", &validateAbc{}).Add("A", &validateAbc{}), "A: error abc; c: cannot validate private field c in Model1."},
		// property not found
		{"t7", &Model1{A: "xyz", c: "abc"}, StructRules{}.Add("d", &validateAbc{}).Add("A", &validateAbc{}), "A: error abc; d: cannot find a field named d in Model1."},
		// nil pointer
		{"t10", (*Model1)(nil), StructRules{}.Add("A", &validateAbc{}), ""},
		// non-struct
		{"t11", 123, StructRules{}.Add("A", &validateAbc{}), "_: only struct or pointer to struct can be validated."},
		// struct tag
		{"t12", Model1{G: "xyz"}, StructRules{}.Add("G", &validateAbc{}), "g: error abc."},
	}
	for _, test := range tests {
		err := test.rules.Validate(test.model)
